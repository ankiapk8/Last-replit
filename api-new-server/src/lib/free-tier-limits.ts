...
export async function checkIsPro(userId: string): Promise<boolean> {
  ...
  try {
    ...
    const row = userRow.rows[0] as
      | { stripe_subscription_id?: string | null; manual_pro?: string | null }
      | undefined;
...
async function getQuotaCount(key: string, metric: string, period: string): Promise<number> {
...
async function incrementQuota(key: string, metric: string, period: string): Promise<number> {
  const result = await db.execute(
    sql`
  ...
  const row = result.rows[0] as { count?: unknown } | undefined;
  return typeof row?.count === "number" ? row.count : parseInt(String(row?.count ?? "1"), 10);
}
...
export async function checkExportQuota(key: string): Promise<{ allowed: boolean }> {
...
/** Throw a standardized FORBIDDEN error for limit violations. */
export function throwLimitError(res: Response, feature: string, message: string): void {
  res.status(403).json({
    error: {
      code: "FORBIDDEN",
      message,
      details: { limitReached: true, feature, requiredPlan: "pro" },
    },
  });
}
