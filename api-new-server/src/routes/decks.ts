...
router.post("/decks", async (req, res, next): Promise<void> => {
  ...
  try {
    ...
    res.status(201).json({ ...deck, cardCount: 0, createdAt: deck.createdAt.toISOString() });
  } catch (err) {
    next(err);
  }
});
...
router.patch("/decks/:id", async (req, res, next): Promise<void> => {
  ...
  const promotingToRoot =
    "parentId" in parsed.data &&
    (parsed.data.parentId === null || parsed.data.parentId === undefined);
  if (promotingToRoot) {
    ...
    if (!patchIsPro) {
      ...
      const existing = await db
        .select({ parentId: decksTable.parentId })
        .from(decksTable)
      ...
      if (wasChild) {
        ...
        if (!allowed) {
          throwLimitError(
            res,
            "deck_count",
            `Free users can have up to ${FREE_TIER.MAX_DECKS} root decks. Upgrade to Pro for unlimited decks.`
          );
          return;
        }
      }
...
router.post("/decks/merge", async (req, res, next): Promise<void> => {
  ...
  if (deckIds.length < 2) {
    res
      .status(400)
      .json({
        error: { code: "VALIDATION_ERROR", message: "Select at least two decks to merge." },
      });
    return;
  }
  if (!newDeckName) {
    res
      .status(400)
      .json({
        error: { code: "VALIDATION_ERROR", message: "A name for the merged deck is required." },
      });
    return;
  }
  ...
  try {
      ...
      res
        ...
        .json({
          error: {
            ...
            message: "Cannot place the merged deck inside a deck that's being merged.",
          },
        });
      return;
    }
    ...
    if (sourceCards.length === 0) {
      res
        .status(400)
        .json({
          error: { code: "VALIDATION_ERROR", message: "Selected decks contain no cards to merge." },
        });
      return;
    }
    ...
    const [mergedDeck] = await db
      ...
      .values({
        name: newDeckName,
        description: `Merged from ${deckIds.length} deck${deckIds.length === 1 ? "" : "s"}: ${deckIds.map((id) => byId.get(id)?.name ?? `#${id}`).join(", ")}`,
        parentId,
      })
      .returning();
