...
router.post("/mobile-auth/token-exchange", async (req: Request, res: Response) => {
  ...
  if (!parsed.success) {
    res
      .status(400)
      .json({
        error: { code: "VALIDATION_ERROR", message: "Missing or invalid required parameters" },
      });
    return;
  }
  ...
  try {
    ...
    const claims = tokens.claims();
    if (!claims) {
      res.status(401).json({ error: { code: "UNAUTHORIZED", message: "No claims in ID token" } });
      return;
    }
    const dbUser = await upsertUser(claims as unknown as Record<string, unknown>);
    const now = Math.floor(Date.now() / 1000);
    ...
    res.status(500).json({ error: { code: "INTERNAL_ERROR", message: "Token exchange failed" } });
  }
});
