...
async function extractPptx(buffer: Buffer): Promise<{ text: string; pageTexts: string[] }> {
  ...
  for (const slidePath of slideFiles) {
    ...
    function walk(obj: unknown): void {
      ...
      if ("a:t" in record) { const t = record["a:t"]; if (typeof t === "string" && t.trim()) texts.push(t.trim()); else if (Array.isArray(t)) t.forEach((s) => { if (typeof s === "string" && s.trim()) texts.push(s.trim()); }); }
...
async function extractDocx(buffer: Buffer): Promise<{ text: string; pageTexts: string[] }> {
  ...
  function walk(obj: unknown): void {
    ...
    if (Array.isArray(obj)) { obj.forEach(walk); return; }
    const record = obj as Record<string, unknown>;
    if ("w:t" in record) { const t = record["w:t"]; if (typeof t === "string" && t.trim()) texts.push(t.trim()); }
    for (const v of Object.values(record)) walk(v);
  }
  walk(parsed);
...
router.post("/extract-office", upload.single("file"), async (req, res): Promise<void> => {
  ...
  try {
    ...
    if (!result.text.trim()) { res.status(422).json({ error: { code: "VALIDATION_ERROR", message: "No readable text found in this file." } }); return; }
    res.json(result);
  } catch (err) {
