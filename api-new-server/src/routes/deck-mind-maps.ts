...
router.post("/decks/:id/mind-maps", async (req, res): Promise<void> => {
  ...
  const { title, data, cardCount } = req.body as {
    title?: string;
    data?: unknown;
    cardCount?: number;
  };
  if (!title || !data) {
    res
      .status(400)
      .json({ error: { code: "VALIDATION_ERROR", message: "title and data are required" } });
    return;
  }
  const [map] = await db
...
router.delete("/decks/:id/mind-maps/:mapId", async (req, res): Promise<void> => {
  ...
  await db
    ...
    .where(and(eq(mindMapsTable.id, mapId), eq(mindMapsTable.deckId, deckId)));
