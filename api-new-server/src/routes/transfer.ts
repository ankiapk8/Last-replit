...
router.get("/export-all-json", async (_req, res, next): Promise<void> => {
  try {
    ...
    if (topLevel.length === 0) {
      res.status(404).json({ error: { code: "NOT_FOUND", message: "No decks to export." } });
      return;
    }
...
router.get("/decks/:id/export-json", async (req, res, next): Promise<void> => {
  ...
  try {
    ...
    res.setHeader("Content-Type", "application/json; charset=utf-8");
    res.setHeader("Content-Disposition", `attachment; filename="${safeName}.ankigen.json"`);
    res.end(JSON.stringify(file, null, 2));
  } catch (err) {
    next(err);
  }
...
router.post("/import-deck-json", async (req, res, next): Promise<void> => {
  try {
    ...
    if (!body || body.format !== "ankigen-deck") {
      res
        .status(400)
        .json({ error: { code: "VALIDATION_ERROR", message: "Not a valid AnkiGen deck file." } });
      return;
    }
    if (typeof body.version !== "number" || body.version > FORMAT_VERSION) {
      res
        .status(400)
        .json({ error: { code: "VALIDATION_ERROR", message: `Unsupported file version.` } });
      return;
    }
    ...
    if (Array.isArray(body.roots)) {
      for (let i = 0; i < body.roots.length; i++) {
        ...
        if (err) {
          res
            .status(400)
            .json({ error: { code: "VALIDATION_ERROR", message: `Invalid file: ${err}` } });
          return;
        }
      }
      ...
      res
        .status(400)
        .json({
          error: { code: "VALIDATION_ERROR", message: "File has no 'root' or 'roots' deck data." },
        });
      return;
    }
    if (inputRoots.length === 0) {
      res
        .status(400)
        .json({
          error: { code: "VALIDATION_ERROR", message: "File contains no decks to import." },
        });
      return;
    }
    ...
    res
      ...
      .json({
        importedName:
          importedNames.length === 1 ? importedNames[0] : `${importedNames.length} top-level decks`,
        importedNames,
