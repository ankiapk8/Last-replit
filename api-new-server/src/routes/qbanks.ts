...
router.get("/qbanks/:id", async (req, res, next): Promise<void> => {
  ...
  try {
    ...
    if (!row || !ownsResource(row.userId ?? null, userId)) {
      res.status(404).json({ error: { code: "NOT_FOUND", message: "QBank not found" } });
      return;
    }
...
router.patch("/qbanks/:id", async (req, res, next): Promise<void> => {
  ...
  const body = req.body as { name?: unknown; description?: unknown; parentId?: unknown };
  const updates: Partial<typeof qbanksTable.$inferInsert> & { updatedAt?: Date } = {
    updatedAt: new Date(),
  };
  ...
  if ("parentId" in body)
    updates.parentId = typeof body.parentId === "number" ? body.parentId : undefined;
  ...
  try {
    const [existing] = await db
      .select({ userId: qbanksTable.userId })
      .from(qbanksTable)
      .where(eq(qbanksTable.id, id));
    if (!existing || !ownsResource(existing.userId ?? null, patchUserId)) {
      res.status(404).json({ error: { code: "NOT_FOUND", message: "QBank not found" } });
      return;
    }
    const [updated] = await db
      .update(qbanksTable)
      .set(updates)
      .where(and(eq(qbanksTable.id, id), qbankOwnerFilter(patchUserId)))
      .returning();
    if (!updated) {
      res.status(404).json({ error: { code: "NOT_FOUND", message: "QBank not found" } });
      return;
...
router.delete("/qbanks/:id", async (req, res, next): Promise<void> => {
  ...
  try {
    ...
    const allQbanks = await db
      .select({ id: qbanksTable.id, parentId: qbanksTable.parentId })
      .from(qbanksTable);
    ...
    const deleted = await db
      .delete(qbanksTable)
      .where(inArray(qbanksTable.id, idsToDelete))
      .returning({ id: qbanksTable.id });
    if (deleted.length === 0) {
      res.status(404).json({ error: { code: "NOT_FOUND", message: "QBank not found" } });
      return;
    }
...
router.patch("/questions/:id", async (req, res, next): Promise<void> => {
  ...
  const updates: Partial<typeof questionsTable.$inferInsert> & { updatedAt?: Date } = {
    updatedAt: new Date(),
  };
