/**
 * AI model selection — Ollama (primary) with OpenRouter fallback.
 *
 * Per-feature model strategy:
 *  - Text generation (decks, cards, explanations)  → qwen3-coder:480b
 *  - Vision (visual card detection from PDF pages)   → llama4:scout
 *  - QBank (MCQ question generation)                  → gpt-oss:120b
 *  - Mind map generation                              → deepseek-v4-flash
 *
 * Priority:
...
export const FREE_TEXT_MODEL = envText ?? "qwen3-coder:480b";

/** Vision model — image-based visual card detection (llama4:scout is multimodal) */
export const FREE_VISION_MODEL = envVision ?? "llama4:scout";

/** QBank model — MCQ question generation */
export const QBANK_MODEL = envQbank ?? "gpt-oss:120b";

/** Mind map model — mind map generation */
export const MINDMAP_MODEL = envMindmap ?? "deepseek-v4-flash";

/** Long-form explanation model — AI Explanation feature (shares text model) */
export const EXPLAIN_MODEL = envText ?? "qwen3-coder:480b";

/** Vision model for detecting figures in PDF page images (shares vision model) */
export const VISUAL_DETECTION_MODEL = envVision ?? "llama4:scout";
