...
export function GenerateForm({
  ...
  const [reviewState, setReviewState] = useState<{
    ...
    autoClose?: () => void;
    stagedCards?: StagedCard[];
    onCommit?: (cards: StagedCard[]) => Promise<void>;
  } | null>(null);
  ...
  const parentOptions = useMemo(
    () =>
      buildParentOptions(
        ((allDecks as DeckWithParent[]) ?? []).filter((d) => (d.kind ?? "deck") !== "qbank")
      ),
    [allDecks]
  );
    ...
    async (file: File) => {
      ...
      if (!isTxt && !isPdf && !isImg && !isPptx && !isDocx) {
        toast({
          ...
          description: `${file.name} is not a supported format (.pdf, .txt, .pptx, .docx, image)`,
          variant: "destructive",
        });
        return;
      }
      ...
      try {
        if (isTxt) {
          ...
          const { text, pageImages, pageTexts, pageImageRegions } = await extractOffice(
            file,
            (progress) => throttledProgressUpdate(id, progress)
          );
          updateFile(id, {
            status: "ready",
            text,
          ...
          const { text, pageImages, pageTexts, pageImageRegions, pageHasVisuals } =
            await extractPdf(buffer, (progress) => throttledProgressUpdate(id, progress));
          ...
          updateFile(id, {
            ...
            pageTexts,
            pageImageRegions,
            pageHasVisuals: pageHasVisuals ?? [],
            progress: "",
            deckType: hasAnyEmbeddedImage ? "both" : "text",
          });
        }
      } catch (error) {
  ...
  const generateOne = (
    ...
    new Promise((resolve, reject) => {
      ...
      const body = JSON.stringify({
        ...
        pageImageRegions:
          pageImageRegions && pageImageRegions.length > 0 ? pageImageRegions : undefined,
      ...
      fetch(apiUrl("api/generate/stream"), {
        ...
        .then(async (resp) => {
          ...
          while (true) {
            ...
            for (const line of lines) {
              ...
              try {
                ...
                if (event.type === "init") {
                  // Store generation ID for polling fallback
                  ...
                  resolve({
                    count: event.generatedCount ?? 0,
                    deckId: event.deck?.id,
                    stagedCards: event.cards,
                  });
                  return;
          ...
          reject(
            new Error(
              "Connection dropped before generation finished. Your network may have changed (Wi-Fi ↔ mobile) or the app was backgrounded. Please try again."
            )
          );
        })
        .catch((err) => {
      ...
      prev.map((f) =>
        f.status === "done"
          ? {
              ...
              generatedDeckId: undefined,
              stagedCards: undefined,
              generatingLog: [],
              generatingPercent: 0,
              generatingMessage: undefined,
              liveCardCount: undefined,
            }
          : f
      )
    );
  ...
  const handleGenerateAll = async () => {
    ...
    const targets = [
      ...readyFiles.map((f) => ({
        ...
        pageImages: f.pageImages,
        pageTexts: f.pageTexts,
        pageImageRegions: f.pageImageRegions,
        deckType: f.deckType,
        visualCardCount: f.visualCardCount,
        customPrompt: fileEffectivePrompt(f),
      })),
      ...(hasManual
        ? [
            {
              id: undefined,
              text: manualText,
              deckName: manualDeckName,
              cardCount: manualCardCount,
              pageImages: [] as string[],
              pageTexts: [] as string[],
              pageImageRegions: [] as ImageRegion[][],
              deckType: "text" as DeckType,
              visualCardCount: "" as number | "",
              customPrompt: manualEffectivePrompt(),
            },
          ]
        : []),
    ];
    ...
    if (!navigator.onLine) {
      ...
      if (textTargets.length > 0) {
        ...
        toast({
          ...
          description: "File-based generation requires internet.",
          variant: "destructive",
        });
      }
    ...
    for (let i = 0; i < targets.length; i++) {
      ...
      try {
        ...
        const wasCancelled =
          cancelledIdsRef.current.has(key) || cancelledIdsRef.current.has(manualCancelKey);
        if (wasCancelled) {
          if (t.id)
            updateFile(t.id, {
              status: "ready",
          ...
          toast({
            title: `Could not generate ${t.deckName}`,
            description: message,
            variant: "destructive",
          });
          fail++;
      ...
      if (i < targets.length - 1 && !cancelledIdsRef.current.has(manualCancelKey))
        await pauseBetweenFiles();
    }
    ...
    if (cancelled > 0 && ok === 0) {
      toast({
        ...
        description: `${cancelled} ${cancelled === 1 ? "deck" : "decks"} were not generated.`,
      });
      return;
    }
        ...
        if (lastStagedCards && lastStagedCards.length > 0) {
          ...
          const commitAndFinish = async (editedCards: StagedCard[]) => {
            try {
              ...
              if (capturedFileId)
                updateFile(capturedFileId, {
                  generatedDeckId: data.deck.id,
                  generatedCount: data.cardCount,
                });
              queryClient.invalidateQueries({ queryKey: getListDecksQueryKey() });
              setInlinePreview(null);
          ...
          setInlinePreview({
            cards: lastStagedCards,
            deckName: capturedName,
            onSave: commitAndFinish,
          });
        } else if (lastDoneId) {
          ...
          >
            ...
            >
              <input
                ...
                accept=".txt,.pdf,.pptx,.docx,.jpg,.jpeg,.png,.webp,.gif"
                multiple
                disabled={isGeneratingAll}
              />
              <motion.div
            ...
            <AnimatePresence initial={false}>
              {files.map((f) => {
                const extractPercent =
                  f.status === "extracting" ? parseProgressPercent(f.progress) : null;
                const extractPhase =
                  f.status === "extracting" ? getProgressPhase(f.progress) : null;
                const phaseLabel =
                  extractPhase === "text"
                    ? "Reading text"
                    : extractPhase === "images"
                ...
                return (
                  ...
                  >
                    <Card
                      className={`border-border/60 border-l-[3px] ${accentClass} shadow-sm bg-card/80 backdrop-blur-sm transition-shadow hover:shadow-md`}
                    >
                      <CardContent className="p-3.5 space-y-2.5">
                        <div className="flex items-center gap-2.5">
                          ...
                          {f.status === "ready" && (
                            <div className="flex items-center gap-1 shrink-0">
                              ...
                              {f.pageImages.length > 0 && (
                                <Badge
                                  variant="outline"
                                  className="text-[10px] gap-1 px-1.5 py-0 h-5 font-normal"
                                >
                                  <ImageIcon className="h-2.5 w-2.5" />
                                  {f.pageImages.length} pg
                                </Badge>
                              )}
                              {(f.pageHasVisuals ?? []).filter(Boolean).length > 0 && (
                                <Badge
                                  variant="outline"
                                  className="text-[10px] gap-1 px-1.5 py-0 h-5 font-normal border-primary/40 text-primary"
                                >
                                  <Sparkles className="h-2.5 w-2.5" />
                                  {(f.pageHasVisuals ?? []).filter(Boolean).length} visual
                                </Badge>
                              )}
                            </div>
                          )}
                          ...
                          {f.status === "done" && (
                            <div className="flex items-center gap-1.5 shrink-0">
                              <Badge className="text-[10px] bg-green-600 hover:bg-green-600 px-1.5 py-0 h-5">
                                {f.generatedCount} cards
                              </Badge>
                              {f.generatedDeckId && (
                                <Button
                                  size="sm"
                          ...
                          (() => {
                            ...
                            return (
                              <div className="space-y-2 pt-0.5">
                                <GenerationStageStepper
                                  activeStage={
                                    f.generatingStage === "extracting"
                                      ? 0
                                      : f.generatingStage === "detecting"
                                        ? 1
                                        : f.generatingStage === "generating"
                                          ? 2
                                          : f.generatingStage === "done"
                                            ? 3
                                ...
                                >
                                  ...
                                  {cancelledIdsRef.current.has(f.id)
                                    ? "Cancelling…"
                                    : "Cancel this deck"}
                                </button>
                              </div>
                        ...
                        {(f.status === "ready" || f.status === "error") && (
                          <div className="space-y-2">
                            ...
                            {f.pageImages.length > 0 && (
                              <div className="space-y-1">
                                ...
                                {f.deckType === "both" && (
                                  <p className="text-[10px] text-muted-foreground">
                                    Creates two decks:{" "}
                                    <span className="font-medium">"{f.deckName} – Text"</span> and{" "}
                                    <span className="font-medium">"{f.deckName} – Visual"</span>.
                                  </p>
                                )}
                              </div>
                            )}
                            ...
                            <div
                              className={`grid gap-2 ${f.pageImages.length > 0 && f.deckType === "both" ? "grid-cols-2" : "grid-cols-1"}`}
                            >
                              {(f.deckType === "text" || f.deckType === "both") && (
                                <div className="space-y-1">
                                  ...
                                  <Input
                                    ...
                                    className="h-7 text-xs"
                                    placeholder={`e.g. ${DEFAULT_TARGET_CARDS}`}
                                    min="1"
                                    max="1000"
                                    disabled={isGeneratingAll}
                                  />
                              ...
                              {(f.deckType === "visual" || f.deckType === "both") &&
                                f.pageImages.length > 0 && (
                                  <div className="space-y-1">
                                    <Label className="text-xs flex items-center justify-between gap-1">
                                      <span className="flex items-center gap-1">
                                    ...
                                    <Input
                                      type="number"
                                      value={f.visualCardCount}
                                      onChange={(e) =>
                                        updateFile(f.id, {
                                          visualCardCount: e.target.value
                                            ? Math.min(Number(e.target.value), 1000)
                                            : "",
                                        })
                                      }
                                      className="h-7 text-xs"
                                      placeholder={`e.g. ${Math.min(f.pageImages.length * 2, 30)}`}
                                      min="1"
                                      max="1000"
                                      disabled={isGeneratingAll}
                                    />
                            ...
                            {(() => {
                              ...
                              const target =
                                typeof f.cardCount === "number"
                                  ? f.cardCount
                                  : DEFAULT_TARGET_CARDS;
                              if (target > capacity && capacity > 0) {
                                return (
                                  <p className="text-[10px] text-emerald-600 dark:text-emerald-500">
                                    Text content likely only supports ~{capacity} cards — target
                                    will be capped.
                                  </p>
                                );
                              }
            ...
            <div className="relative pt-1">
              ...
              <div className="space-y-1.5 pt-5">
                <Textarea
                  placeholder="Paste study material, notes, or an article here…"
              ...
              {manualText.trim().length > 0 && (
                ...
                >
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setManualCardCount(
                            e.target.value ? Math.min(Number(e.target.value), 1000) : ""
                          )
                        }
                        className="h-7 text-xs"
                        placeholder={`e.g. ${DEFAULT_TARGET_CARDS}`}
                  ...
                  {(() => {
                    ...
                    const target =
                      typeof manualCardCount === "number" ? manualCardCount : DEFAULT_TARGET_CARDS;
                    if (target > capacity && capacity > 0) {
                      return (
                        <p className="text-[10px] text-emerald-600 dark:text-emerald-500">
